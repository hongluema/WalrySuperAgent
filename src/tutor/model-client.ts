  ...
  async planBookTurn(input: { message: string; state: TutorState; material?: string; now: string }, signal?: AbortSignal): Promise<BookTurn> {
    return generateJson({
      ...
      contract: {
        ...
        currentChapterNumber: "当前第几章，未知时为 1", totalChapters: "有可靠目录时填全书总章数，否则为 0",
        ...
        choice: { id: "稳定问题id", prompt: "只问一个必要问题", options: [{ id: "option-1", label: "选项" }, { id: "option-2", label: "选项" }], custom: { id: "custom", label: "自定义", placeholder: "请补充" } },
    ...
    if (input.state.sessionMode === "explain") {
      const result = streamText({
        ...
        system: withAgentRules([
          ...
          "最后用一句可记忆的话收束。",
          "Markdown 标记（#、-、1.、>）与正文之间必须有空格。禁止 `####①`、`1.没有空格`、`-没有空格`。最多用到三级标题。加粗用成对的 `**文字**`，代码块用成对的 ```。",
